...
echo -e "${YELLOW}[2/5] Installing Cloudflare Tunnel...${NC}"
...
echo -e "${YELLOW}[4/5] Loading auth keys...${NC}"

KEYS_FILE=".env.keys"
if [[ ! -f "$KEYS_FILE" ]]; then
	echo "Could not find .env.keys file."
	echo ""
	echo "Run MSW-Tools to generate keys:"
...
echo -e "${YELLOW}[5/5] Updating .env and starting services...${NC}"
...
echo ""
echo "  Staging uses local files from ../MSW-Tools/data/"
echo ""
echo "=========================================="
